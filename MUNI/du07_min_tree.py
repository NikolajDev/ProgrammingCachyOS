...
def _build_node(leaves: list[int], lo: int, hi: int, height: int) -> Node:
    if hi - lo == 1:
        return Node(leaves[lo])
    left_count = 2 ** (height - 1)
    mid = lo + left_count
    left = _build_node(leaves, lo, mid, height - 1)
    right = _build_node(leaves, mid, hi, height - 1)
    return Node(min(left.key, right.key), left, right)
...
def build_min_tree(leaves: list[int]) -> BinTree:
    ...
    return BinTree(_build_node(leaves, 0, n, height))
...
def _subtree_height(node: Node) -> int:
    h = 0
...
def check_left_align(tree: BinTree) -> bool:
    ...
    while stack:
        ...
        if node.left is None and node.right is None:
            continue
        # podmínka a): jediný potomek musí být levý
        if node.left is None:
            return False
        # podmínka b): oba potomci → levý musí být perfektní strom
        if node.right is not None:
            lh = _subtree_height(node.left)
            expected_nodes = 2 ** (lh + 1) - 1
            if _count_nodes(node.left) != expected_nodes:
                return False
...
def check_min(tree: BinTree) -> bool:
    ...
    while stack:
        ...
        if node.left is None:
            return False
        expected = node.left.key
        if node.right is not None:
            expected = min(expected, node.right.key)
