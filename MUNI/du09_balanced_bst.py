...
def _verify_size(node: Node | None) -> int | None:
    if node is None:
        return 0
    left_size = _verify_size(node.left)
    right_size = _verify_size(node.right)
...
def check_size(tree: BSTree) -> bool:
    ...
    if tree.root is None:
        return True
    return _verify_size(tree.root) is not None


def _is_3_5_balanced(node: Node | None) -> bool:
    if node is None:
        return True
    limit = 3 * node.size
    if node.left and 5 * node.left.size > limit:
        return False
    if node.right and 5 * node.right.size > limit:
        return False
    return _is_3_5_balanced(node.left) and _is_3_5_balanced(node.right)
...
def check_3_5_balanced(tree: BSTree) -> bool:
    ...
    extra prostorová složitost: O(h), kde ‹h› je výška stromu
        (Do extra prostorové složitost nepočítáme velikost vstupu, ale
         počítáme do ní zásobník rekurze.)
    """
    return _is_3_5_balanced(tree.root)
...
def insert(tree: BSTree, key: int) -> Node | None:
    ...
    for node in path:
        ...
        if 5 * l_size > limit or 5 * r_size > limit:
            return node
...
def _collect_inorder(u: Node | None, nodes: list[Node]) -> None:
    if u:
        _collect_inorder(u.left, nodes)
        nodes.append(u)
        _collect_inorder(u.right, nodes)


def _build_balanced(nodes: list[Node], start: int, end: int,
                    parent: Node | None) -> Node | None:
    if start > end:
        return None
    mid = (start + end) // 2
    root = nodes[mid]
    root.parent = parent
    root.left = _build_balanced(nodes, start, mid - 1, root)
    root.right = _build_balanced(nodes, mid + 1, end, root)

    l_s = root.left.size if root.left else 0
    r_s = root.right.size if root.right else 0
    root.size = 1 + l_s + r_s
    return root
...
def rebalance(tree: BSTree, node: Node) -> None:
    ...
    """
    nodes = []
    _collect_inorder(node, nodes)
    orig_parent = node.parent

    new_subtree_root = _build_balanced(nodes, 0, len(nodes) - 1, orig_parent)

    if orig_parent is None:
        tree.root = new_subtree_root
    elif orig_parent.left == node:
        orig_parent.left = new_subtree_root
    else:
        orig_parent.right = new_subtree_root
